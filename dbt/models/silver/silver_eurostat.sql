{{ config(materialized='incremental', unique_key=['dataset','country_code','year','month'], on_schema_change='fail', tags=['silver','eurostat']) }}
with source as (
...
deduplicated as (
    select source, dataset, description, upper(trim(country_code)) as country_code,
