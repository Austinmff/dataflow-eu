...
FAKE_ECB_RESPONSE = {
    ...
    "structure": {"dimensions": {"observation": [{"id": "TIME_PERIOD", "values": [{"id": "2023-01", "name": "2023-01"}, {"id": "2023-02", "name": "2023-02"}]}]}}
}
...
def s3_bucket():
    with mock_aws():
        ...
        client.create_bucket(Bucket="test-bronze-bucket", CreateBucketConfiguration={"LocationConstraint": "eu-west-1"})
        yield client
...
class TestECBExtractorParsing:
    def test_parse_response_returns_flat_records(self):
        ...
        records = extractor._parse_response(FAKE_ECB_RESPONSE, "EXR.M.USD.EUR.SP00.A", meta, year=2023, month=1)
...
class TestECBExtractorS3:
    ...
    def test_upload_writes_correct_payload(self, s3_bucket):
        ...
        records = [{"source": "ecb", "series_key": "EXR.M.USD.EUR.SP00.A", "description": "EUR/USD", "period": "2023-01", "year": 2023, "month": 1, "value": 1.0823, "unit": "USD"}]
